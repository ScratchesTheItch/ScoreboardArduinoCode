...
#include "glcdfont_GCScoreboard.c"
...
// draw a skinny character 
void HT1632LEDMatrix::drawSkinnyChar(uint8_t x, uint8_t y, char c, uint16_t color, uint8_t size) {
    for (uint8_t i =0; i<3; i++ ) {
        uint8_t line = pgm_read_byte(skinnyfont+(c*3)+i);
        for (uint8_t j = 0; j<8; j++) {
            if (line & 0x1) {
                if (size == 1) // default size
                    drawPixel(x+i, y+j, color);
...
// draw a sideways character - anchor point is upper right side
void HT1632LEDMatrix::drawSideChar(uint8_t x, uint8_t y, char c, 
                                   uint16_t color, uint8_t size) {
    for (uint8_t i =0; i<5; i++ ) {
        uint8_t line = pgm_read_byte(font+(c*5)+i);
        for (uint8_t j = 0; j<8; j++) {
            if (line & 0x1) {
                if (size == 1) // default size
                    drawPixel(x+j, y+5-i, color);
                else {  // big size
                    fillRect(x+j*size, y+(5-i)*size, size, size, color);
                } 
            }
            line >>= 1;
        }
...
// draw a sideways skinny character - anchor point is upper right side
void HT1632LEDMatrix::drawSideSkinnyChar(uint8_t x, uint8_t y, char c, 
                                    uint16_t color, uint8_t size) {
    for (uint8_t i =0; i<3; i++ ) {
        uint8_t line = pgm_read_byte(skinnyfont+(c*3)+i);
        for (uint8_t j = 0; j<8; j++) {
            if (line & 0x1) {
                if (size == 1) // default size
                    drawPixel(x+j, y+3-i, color);
                else {  // big size
                    fillRect(x+j*size, y+(3-i)*size, size, size, color);
                } 
            }
            line >>= 1;
        }
...
void HT1632LEDMatrix::drawLogo (uint8_t x, uint8_t y, 
                                 const uint8_t *bitmap, uint8_t w, uint8_t h,
                                     uint8_t color) {
        for (uint8_t j=0; j<h; j++) {
            for (uint8_t i=0; i<w; i++ ) {
                if (*(bitmap + i + (j/8)*w) & _BV(j%8)) {
                    drawPixel(x+j, y+w-i, color);
                }
            }
        }
    }
